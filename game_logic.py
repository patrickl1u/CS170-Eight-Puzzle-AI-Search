    ...
    if misplaced_tile_heuristic:
        ...
        for x in range(len(b2)):
            ...
            for y in range(len(b2[x])):  # go through each column in each row
                ...
                counter_check += 1
                if (counter_check == 9
                    ):  # when it's 9, revert back to 0 because we dont have 9
                    counter_check = 0
    ...
    if euclidean_distance_heuristic:
        ...
        for x in range(len(b2)):
            for y in range(len(b2[x])):
                counter_check = (x * len(b2[x]) + y + 1) % 9 # cuz we don't actually have a 0
                        ...
                        for findIndexY in range(len(b2[findIndexX])):
                            if b2[findIndexX][findIndexY] == counter_check:
                                ...
                                if a != 0 and b != 0:
                                    ...
                                    c = max(a, b) # little optimization cuz y not
...
class gameBoard:
    ...
    def is_swap_valid(self, location1, location2):
        ...
        if ((location1_x - 1 == location2_x or location1_x + 1 == location2_x
             or location1_x == location2_x) and
            (location1_y - 1 == location2_y or location1_y + 1 == location2_y
             or location1_y == location2_y)):
            if ((location1_x + 1 == location2_x)
                    and (location1_y + 1 == location2_y)):
...
class gameMove:
    ...
    def new_gameMoves(self):
        ...
        for (dir, boardafter, cost) in validmoves:
            gameMoveList.append(
                gameMove(self, gameBoard(boardafter), dir, self.depth + 1,
                         self.cost + cost))

        return gameMoveList

    def is_solution(self):
        return np.array_equal(self.boardAfterMove.board,
                              np.array(np.mat('1 2 3; 4 5 6; 7 8 0')))
...
def solve(startingBoard, algorithm="ucs"):
    ...
    firstNode = gameMove(gameMove(None, startingBoard, "start", 0, 0),
                         startingBoard, "start", 0, 0)
    ...
    queue_histmaxsize = 1 # take first node from queue? or define 0 from start # frontier.qsize()

    # see if start move is end goal
    if (firstNode.is_solution()):
            # print("found!")
            # todo: return queue stats
            return firstNode, (nodes_expanded, queue_histmaxsize)
    ...
    while (frontier.empty() is False):
        ...
        if (move.is_solution()):
            # seen_moves = {}
            # print("found!")
            # want to look through lower cost moves, current could be suboptimal
            while(frontier.empty() is False):
                
                prio2, move2 = frontier.get()
                # move2.print()
                move2_id = move2.get_id()
                # print(prio)
                # print(prio2)
                if prio2 > prio:
                    continue
                if(move2_id in seen_moves):
                    continue
                if(move2.is_solution()):
                    move = move2
                    prio = prio2
                    continue
                else:
                    # print("true")
                    seen_moves[move2_id] = True
                moves2 = move2.new_gameMoves()
                for newmove2 in moves2:
                    if(newmove2.get_id() not in seen_moves):
                        frontier.put((newmove2.cost, newmove2))
